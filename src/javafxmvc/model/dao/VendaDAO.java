...
public class VendaDAO {
    ...
    public boolean alterar(Venda venda) {
        ...
        try {
          ...
          List<ItemDeVenda> itensDeVenda = itemDeVendaDAO.listarPorVenda(venda);
           for(ItemDeVenda iv : itensDeVenda){
            ...
            stmt.execute();    
           for(ItemDeVenda iv: venda.getItensDeVenda()){//Marcos
               Produto p = iv.getProduto();
               p.setQuantidade(p.getQuantidade() - iv.getQuantidade());
               produtoDAO.alterar(p);
               itemDeVendaDAO.inserir(iv);
           }
            connection.commit(); //Marcos
            return true;
        } catch (SQLException ex) {
            Logger.getLogger(VendaDAO.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        ...
        try {
            ...
            while (resultado.next()) {
                ...
                if (!retorno.containsKey(resultado.getInt("ano")))
                {
                    ...
                    linha.add(resultado.getInt("count(cdVenda)"));
                    retorno.put(resultado.getInt("ano"), linha);
                }else{
                    ArrayList linhaNova = retorno.get(resultado.getInt("ano"));
                    linhaNova.add(resultado.getInt("mes"));
                    linhaNova.add(resultado.getInt("count(cdVenda)"));
                }
            }
            return retorno;
        } catch (SQLException ex) {
